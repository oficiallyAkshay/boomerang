...
reaches the network: the tests that exercise downloading replace
``clean.urlopen`` and ``clean.socket.gethostbyname``, and two tests assert that
the offline paths would explode if they tried to open a socket.
