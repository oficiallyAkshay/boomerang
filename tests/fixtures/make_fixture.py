...
def make_vendor_samples(out_dir: Path) -> None:
    """One sample per vendor, for template work and cleaner tests."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    _, files = _build(1)
    pairs = {
        "lyft.html": f"{RIDS['lyft1']}.html",
        "doordash.html": f"{RIDS['dd1']}.html",
        "united.html": f"{RIDS['flight']}.html",
        "uber.html": f"{RIDS['scooter']}.html",
        "plaintext.txt": f"{RIDS['hotel_conf']}.txt",
    }
