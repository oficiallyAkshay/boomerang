...
receipts dir as `<rid>.html | .txt | .pdf | .png | .jpg`. amt has at most 2 decimals. desc must not
contain an em dash or the standalone word "due".

## build.py

```text
validate(data: dict) -> list[str]            # empty list means valid; each entry is a human-readable problem
...
totals(data: dict) -> dict                   # {"days": [(label, subtotal)], "expenses": x, "stipend": y, "total": z}
...
CLI: clean.py IN.html --out OUT.html [--vendor NAME] [--vendors DIR] [--images DIR]
...
fetch_all(source, queries: list[Query], out_dir: Path) -> list[str]   # source.search(query_str)->list[str]; source.get(rid)->{"html": str|None, "text": str|None, "from": str, "subject": str, "date": str, "attachments": [(filename, bytes)]}; writes <rid>.html/.txt/.meta.json and attachments as <rid>.<n>.<ext>; one message at a time; skips rids already on disk
